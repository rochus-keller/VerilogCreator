...
Note that on Windows you also have to compile QtCreator itself because the binary versions deployed by the Qt Company don't include the lib files for the plugin dll's (i.e. Core.lib, TextEditor.lib, ProjectExplorer.lib, Aggregation.lib, ExtensionSystem.lib, QtcSsh.lib and Utils.lib are missing). Compiling QtCreator is not an easy task and unfortunately there seems not to be no work-around.
